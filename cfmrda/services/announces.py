...
from cfmrda.services import auth_service
...
class AnnouncesService():
    ...
    def __get_announce_for_edit(self, callsign, ann_id):
        announces = self.__read_file()
        for i, announce in enumerate(announces):
            if ann_id == announce['id']:
                if announce['user'] == callsign or auth_service.is_admin(callsign):
                    return (announces, i)
                raise response_error_unauthorized()
        raise web.HTTPNotFound()
    ...
    async def create(self, callsign, data):
        ...
        data['announce']['id'] = str(uuid.uuid1())
    ...
    async def update(self, callsign, data):
        anns, edit_index = self.__get_announce_for_edit(callsign, data['announce']['id'])
        anns[edit_index] = data['announce']
        self.__write_file(anns)
        return response_ok()

    async def delete(self, callsign, data):
        anns, edit_index = self.__get_announce_for_edit(callsign, data['id'])
        del anns[edit_index]
        self.__write_file(anns)
        return response_ok()
